...
func TestThomann_LoadProducts(t *testing.T) {
	...
	t.Run("parse all products on a product page", func(t *testing.T) {
		...
		assert.Equal(t, "In 1-2 Wochen lieferbar", prds[0].AvailabilityInfo)
		assert.Equal(t, 4, prds[0].AvailabilityScore)
		...
		assert.Equal(t, "In 7-9 Wochen lieferbar", prds[1].AvailabilityInfo)
		assert.Equal(t, 4, prds[1].AvailabilityScore)
